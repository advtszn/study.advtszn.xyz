 ...
 * `bun add --no-save`. Lets bun resolve compatible versions across plugins.
 */
export function installNativeDeps(
  ...
  try {
    execSync(`bun add --no-save ${installArgs.join(" ")}`, {
      cwd: process.cwd(),
...
function buildInstalledPlugin(pluginDir: string, name: string, verbose?: boolean): void {
  ...
  try {
    ...
    // bun has no `prune` and a production install keeps existing devDependencies,
    // so reinstall from scratch without them
    fs.rmSync(path.join(pluginDir, "node_modules"), { recursive: true, force: true })
    execSync("bun install --production --ignore-scripts", {
      cwd: pluginDir,
      stdio: verbose ? "inherit" : "pipe",
      timeout: 60_000,
    })
